...
st.title("세븐스플릿 백테스팅에 오신걸 환영합니다.")
st.subheader("무엇을 하실건가요?")
select_what_for = st.radio("", ["1. 나의 전략 백테스팅", '2. 최적전략 찾기', '3. 포트폴리오 구성하기'])


if select_what_for == '2. 최적전략 찾기'    :
    want_to_contribute = st.button("시작하기", key=1)
    if want_to_contribute:
        switch_page("최적전략 찾기")

elif select_what_for == '3. 포트폴리오 구성하기'    :
    want_to_contribute = st.button("시작하기", key=2)
    if want_to_contribute:
        switch_page("포트폴리오 구성하기")

elif select_what_for == '1. 나의 전략 백테스팅'    :
    if st.button("시작하기",key=3) :
